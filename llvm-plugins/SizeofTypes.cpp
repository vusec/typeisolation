...
Type *SizeofTypes::getSizeofType(CallBase *CI) {
    return mallocTypes.lookup(CI);
}

void SizeofTypes::setSizeofType(CallBase *CI, Type *Ty) {
    mallocTypes[CI] = Ty;
}
...
bool SizeofTypes::runOnModule(Module &M) {
    unsigned Count = 0;
    DenseMap<CallBase*, Type*> Propagate;

    for (Function &F : M) {
        for (Instruction &I : instructions(F)) {
            ifncast(CallBase, CI, &I)
                continue;
            ...
            ifcast(MemSetInst, MI, CI) {
                ifcast(CallBase, Dst, MI->getDest()) {
