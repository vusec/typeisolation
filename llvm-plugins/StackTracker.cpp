...
struct StackTracker : public FunctionPass {
	...
	BasicBlock &bb = F.getEntryBlock();
        IRBuilder<> AllocaBuilder(&bb, bb.getFirstInsertionPt());
        ...
        Instruction *metaEntry = NULL;
        
        AllocaInst *firstAlloca = NULL;
        /*if (!FixedCompression && interestingAllocas.size() > 1) {
            firstAlloca = *(interestingAllocas.begin());
            if (staticAllocas.count(firstAlloca) != 0) {
                IRBuilder<> B(firstNonAlloca);
                std::vector<Value *> callParams;
                callParams.push_back(B.CreatePtrToInt(firstAlloca, IntPtrTy));
                metaEntry = B.CreateCall(MetabasegetFunc, callParams);
            }
        }*/
        ...
        for (auto AI : interestingAllocas) {
            ...
            unsigned long elementSize = DL->getTypeAllocSize(AI->getAllocatedType());
            Value *size;
            if (AI->isArrayAllocation()) {
                Value *elemCount = B.CreateZExt(AI->getArraySize(), IntPtrTy);
                size = B.CreateMul(elemCount, ConstantInt::get(IntPtrTy, elementSize));
            }
            else {
                size = ConstantInt::get(IntPtrTy, elementSize);
            }

            Value *ptr = B.CreatePtrToInt(AI, IntPtrTy);
            ...
            if (!DeepMetadata) {
                ...
                if (!FixedCompression) {
                    callParams.push_back(ConstantInt::get(IntPtrTy, alignment));
                }
                if (FixedCompression || metaEntry == NULL) {
                    B.CreateCall(MetasetFunc, callParams);
                } else {
                    callParams.push_back(metaEntry);
                    callParams.push_back(B.CreatePtrToInt(firstAlloca, IntPtrTy));
                    B.CreateCall(MetasetFastFunc, callParams);
                }
            } else {
