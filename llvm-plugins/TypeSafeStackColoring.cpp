...
#define DEBUG_TYPE "typesafestackcoloring"

// Disabled by default due to PR32143.
static cl::opt<bool> ClColoring("type-safe-stack-coloring",
                                cl::desc("enable type safe stack coloring"),
                                cl::Hidden, cl::init(false));

const TypeSafeStackColoring::LiveRange &TypeSafeStackColoring::getLiveRange(AllocaInst *AI) {
  const auto IT = AllocaNumbering.find(AI);
  assert(IT != AllocaNumbering.end());
  return LiveRanges[IT->second];
}

bool TypeSafeStackColoring::readMarker(Instruction *I, bool *IsStart) {
...
void TypeSafeStackColoring::removeAllMarkers() {
  for (auto *I : Markers) {
...
void TypeSafeStackColoring::collectMarkers() {
...
void TypeSafeStackColoring::calculateLocalLiveness() {
  bool changed = true;
  while (changed) {
    changed = false;
...
void TypeSafeStackColoring::calculateLiveIntervals() {
...
LLVM_DUMP_METHOD void TypeSafeStackColoring::dumpAllocas() {
  dbgs() << "Allocas:\n";
  for (unsigned AllocaNo = 0; AllocaNo < NumAllocas; ++AllocaNo)
    dbgs() << "  " << AllocaNo << ": " << *Allocas[AllocaNo] << "\n";
}

LLVM_DUMP_METHOD void TypeSafeStackColoring::dumpBlockLiveness() {
  dbgs() << "Block liveness:\n";
  for (auto IT : BlockLiveness) {
    BasicBlock *BB = IT.getFirst();
    BlockLifetimeInfo &BlockInfo = BlockLiveness[BB];
...
LLVM_DUMP_METHOD void TypeSafeStackColoring::dumpLiveRanges() {
...
void TypeSafeStackColoring::run() {
