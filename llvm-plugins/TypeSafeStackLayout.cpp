...
#define DEBUG_TYPE "typesafestacklayout"

static cl::opt<bool> ClLayout("type-safe-stack-layout",
                              cl::desc("enable type safe stack layout"), cl::Hidden,
                              cl::init(true));

LLVM_DUMP_METHOD void TypeSafeStackLayout::print(raw_ostream &OS) {
  OS << "Stack regions:\n";
...
void TypeSafeStackLayout::addObject(const Value *V, unsigned Size, unsigned Alignment,
...
void TypeSafeStackLayout::layoutObject(StackObject &Obj) {
  if (!ClLayout) {
  ...
  if (End > LastRegionEnd) {
    ...
    if (Start > LastRegionEnd) {
      LLVM_DEBUG(dbgs() << "  Creating gap region: " << LastRegionEnd << " .. "
                        << Start << "\n");
      Regions.emplace_back(LastRegionEnd, Start, TypeSafeStackColoring::LiveRange());
      LastRegionEnd = Start;
    }
...
void TypeSafeStackLayout::computeLayout() {
